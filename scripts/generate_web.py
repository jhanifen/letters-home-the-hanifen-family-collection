...
/* Handwriting image reveal — wipes left to right */
.hw-reveal {
  overflow: hidden;
  margin-bottom: 20px;
  clip-path: inset(0 100% 0 0);
  animation: hw-wipe 4s cubic-bezier(0.25, 0.1, 0.25, 1) 1s forwards;
}
@keyframes hw-wipe {
  ...
  55%  { clip-path: inset(0 28% 0 0); }
  80%  { clip-path: inset(0 10% 0 0); }
  85%  { clip-path: inset(0 8% 0 0); }
...
/* Typed text appears word by word */
.hw-typed {
  text-align: center;
  margin-top: 4px;
  line-height: 1.7;
...
.hw-word {
  ...
  color: #d4c5a9;
  opacity: 0;
  animation: hw-fade 0.5s ease forwards;
  animation-delay: calc(1.2s + var(--d) * 0.45s);
  margin: 0 3px;
}
.hw-word-name {
